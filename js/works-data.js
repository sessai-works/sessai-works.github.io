...
var DASHBOARD_DATA = {
  ...
  "projects": [
    ...
    },
    {
      ...
      "detail": {
        "overview": "支援先企業の正社員18名が、自分の担当クライアントの稼働状況を1画面で見られるようにする。kintoneの「稼働請求時間」アプリ（日々の作業記録）と「契約」アプリ（誰がどの会社の担当か）を毎週月曜のお昼に読み、担当者ごとに集計してSharePointのリストへ書き出す（急ぐときは更新ボタンを押せばすぐ更新される）。1行に契約時間・請求時間・消化率・実働時間・自分の実働・自分の実働率が並ぶので、契約の枠を使いすぎ／使わなすぎかと、その会社が自分の手を離れているかが同時に分かる。全員が同じリンクを開けば、それぞれ自分の担当分だけが出る。過去の月は別のリストに積み上がる。kintone本体には一切手を入れない（読み取りだけ）。",
        ...
        "tools": [
          ...
          "Windowsのタスクスケジューラ（毎週月曜12時の自動実行＋更新ボタンの見張り）"
        ],
        "impact": {
          ...
          "after": "**読み方の案内ページを開くだけ**で、上に「このデータは◯月◯日◯時 時点です」、下に自分の担当分だけの表が出る。契約時間・請求時間・消化率・実働時間・自分の実働・自分の実働率・請求差異が1行に並ぶ。消化率は「数字／今月記録なし／枠なし」を出し分け、色は月の進み具合と比べて付く。数字はkintoneの集計画面と同条件で109社すべて一致することを確認ずみ。更新は毎週月曜のお昼に自動（急ぐときは更新ボタン）。過去の月は別のリストに積み上がり、月が変わった週に先月ぶんを締め直す。",
          "savedTime": ""
        }
      }
    }
