...
var DASHBOARD_DATA = {
  ...
  "projects": [
    ...
    },
    {
      ...
      "category": "report",
      "status": "testing",
      "description": "自分が担当しているクライアントの「今月ここまでの請求時間・契約時間・消化率・実働時間」を1画面で見られるようにする。kintoneの2つのアプリを毎朝読み、社内のSharePointリストへ書き出す",
      "detail": {
        "overview": "支援先企業の正社員18名が、自分の担当クライアントの稼働状況を1画面で見られるようにする。kintoneの「稼働請求時間」アプリ（日々の作業記録）と「契約」アプリ（誰がどの会社の担当か）を毎朝読み、担当者ごとに集計してSharePointのリストへ書き出す。1行に契約時間・請求時間・消化率・実働時間・自分の実働・自分の実働率が並ぶので、契約の枠を使いすぎ／使わなすぎかと、その会社が自分の手を離れているかが同時に分かる。全員が同じリンクを開けば、それぞれ自分の担当分だけが出る。過去の月は別のリストに積み上がる。kintone本体には一切手を入れない（読み取りだけ）。",
        "background": "岩本さんからの相談が発端。kintoneには集計表が12本あるが「自分の担当ぶんだけに絞る」ことができず、担当クライアントの状況を見るには1社ずつ開き直す必要があった。担当者の情報は契約アプリにしかなく、作業記録の「責任者」欄では今月210社中63社が空欄で、絞り込んでも4社しか出ない（正解は12社）。そのため2つのアプリを顧客コードで結合する形にした。試験運用に入ってから、澤田さんの要望でG契約も対象に広げ、「工数のうち自分が手を動かした時間」とその割合も出すようにした（担当先が自分の手を離れているかを見るため）。",
        "tools": [
          "kintone（稼働請求時間アプリ・契約アプリ／閲覧のみ・書き込みなし）",
          "Python（毎朝データを作り直す・品質ゲート付き）",
          "SharePointリスト（社内の置き場。ログインした人だけが見られる）",
          "PnP PowerShell（SharePointへの書き込み）",
          "Windowsのタスクスケジューラ（毎朝の自動実行）"
        ],
        "impact": {
          ...
          "after": "毎朝自動で作り直され、リンクを開くだけで自分の担当分だけがまとまって見られる。契約時間・請求時間・消化率・実働時間・自分の実働・自分の実働率・請求差異が1行に並ぶ。数字はkintoneの集計画面と同条件で109社すべて一致することを確認ずみ",
          "savedTime": ""
        }
      }
    }
