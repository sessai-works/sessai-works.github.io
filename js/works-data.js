...
var DASHBOARD_DATA = {
  ...
  "projects": [
    ...
    },
    {
      ...
      "detail": {
        "overview": "支援先企業の正社員19名が、自分の担当クライアントの稼働状況を1画面・1操作で見られるようにする。kintoneの「稼働請求時間」アプリ（日々の作業記録）と「契約」アプリ（誰がどの会社の担当か）を毎朝読み、担当者ごとに集計してSharePointのリストへ書き出す。契約時間に対する消化率も出すので、使いすぎ・使わなすぎがその場で分かる。kintone本体には一切手を入れない（読み取りだけ）。",
        "background": "岩本さんからの相談が発端。kintoneには集計表が12本あるが「自分の担当ぶんだけに絞る」ことができず、担当クライアントの状況を見るには1社ずつ開き直す必要があった。担当者の情報は契約アプリにしかなく、作業記録の「責任者」欄では今月210社中63社が空欄で、絞り込んでも4社しか出ない（正解は12社）。そのため2つのアプリを顧客コードで結合する形にした。",
        "tools": [
        ...
        "impact": {
          ...
          "after": "毎朝自動で作り直され、担当者ごとにまとまった一覧をブラウザで開くだけで見られる。請求時間・契約時間・消化率・実働時間・請求差異が1行に並ぶ。数字はkintoneの集計画面と同条件で109社すべて一致することを確認ずみ",
          "savedTime": ""
        }
      }
    }
