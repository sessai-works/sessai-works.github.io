...
var DASHBOARD_DATA = {
  ...
  "projects": [
    ...
    },
    {
      ...
      "category": "report",
      "status": "testing",
      "description": "マネーフォワードの損益計算書（月次推移）を添付すると、数値をコードで確定させた月次PL分析レポートを返し、そのあと内容について質問できるツール（freee・弥生は準備中）",
      "detail": {
        "overview": "カスタムGPT「MF月次PLアナライザー」の再設計版。**現在の対応はマネーフォワードのみ**で、freee・弥生のファイルを添付した場合は分析せずに「準備中です」と表示して止まる（黙って誤った数字を出さないため）。マネーフォワードの損益計算書（月次推移）CSVを添付すると、9つの節で構成された月次PL分析レポートを返す。最大の特徴は「数値はコードが確定させ、AIは文章だけを書く」構造で、AIには金額を直接書かせず引換券（プレースホルダ）だけを書かせ、コードが確定値に置き換える。会計ソフトの合計と明細の合算が1円でもズレたら処理を止める（fail-closed）。レポートが出たあとは、同じ画面で「人件費は売上の何％か」「販管費が増えたのは何が原因か」といった質問に答えられる（計算7種）。1つの会話につき1社に限定する構造にして、他社の数値が混ざる事故を仕組みで防いでいる。",
