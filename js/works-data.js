  ...
  "projects": [
    ...
    },
    {
      ...
      "detail": {
        ...
        "background": "依頼者の方からの相談が発端。kintoneには集計表が12本あるが「自分の担当ぶんだけに絞る」ことができず、担当クライアントの状況を見るには1社ずつ開き直す必要があった。担当者の情報は契約アプリにしかなく、作業記録の「責任者」欄では今月210社中63社が空欄で、絞り込んでも4社しか出ない（正解は12社）。そのため2つのアプリを顧客コードで結合する形にした。試験運用に入ってから、別の社員の方の要望でG契約も対象に広げ、「工数のうち自分が手を動かした時間」とその割合も出すようにした（担当先が自分の手を離れているかを見るため）。",
        "tools": [
        ...
        "impact": {
          ...
          "after": "毎週月曜のお昼に自動で作り直され（急ぐときは更新ボタン）、リンクを開くだけで自分の担当分だけがまとまって見られる。契約時間・請求時間・消化率・実働時間・自分の実働・自分の実働率・請求差異が1行に並ぶ。数字はkintoneの集計画面と同条件で109社すべて一致することを確認ずみ",
          "savedTime": ""
        }
      }
    }
