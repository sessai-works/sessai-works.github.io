...
var DASHBOARD_DATA = {
  ...
  "projects": [
    ...
    {
      "name": "議事録作成メーカー（支援先企業 クライアント支援部門用）",
      "nameEn": "Meeting Minutes Maker (支援先企業 Client Support Team)",
      "category": "document",
      "status": "live",
      "description": "会議の文字起こし(Word)を送ると、クライアント向けと社内向けの二層議事録を自動で作り、SharePointに保存してTeamsで知らせるツール。支援先企業のクライアント支援部門用の受付窓口です",
      "detail": {
        "overview": "会議の文字起こし(Word)から、クライアント向けと社内向けの二層議事録を自動生成し、SharePoint保存・Teams通知まで一気通貫で行うツール。2026年7月に本番稼働を開始。受付フォームは窓口ごとに分かれていますが、裏側のしくみ（AI・保存・通知）は3窓口で共用しています。このカードは支援先企業 クライアント支援部門用の窓口です。",
    ...
    },
    {
      ...
      "nameEn": "Meeting Minutes Maker (Partner Tax Firm)",
      "category": "document",
      "status": "live",
    ...
    },
    {
      ...
      "detail": {
        "overview": "支援先企業社員からの経理・労務・総務・契約・システム等バックオフィス質問を、Microsoft Forms入力→Power Automateによる質問の登録と通知→SharePointへ構造化蓄積→情シスのご担当者へTeams通知→回答管理、までを自動化します。最終的には既存社内AIチャットボット「社内AIチャットボット」が過去ナレッジを参照して自動回答する状態を目指し、Phase1 蓄積→Phase2 検索→Phase3 AI自動回答の3段階で段階移行します。",
        ...
        "impact": {
          ...
          "after": "全質問が構造化された状態でSharePointに自動蓄積。Phase1は情シスのご担当者に通知が一括集約され、SLA違反は日次09:00に自動検知。情シスのご担当者がナレッジ化判断時にAIBlockedフラグ（AI投入可否）とKnowledgeBody（汎用化済み本文）を設定すれば、日次23:00にナレッジtxtが自動生成され、Phase3で自動回答化を実現予定。",
    ...
    },
    {
      ...
      "category": "management",
      "status": "testing",
      "description": "業務管理ツールの月次業務データから、担当クライアントの作業を1枚のHTMLガントチャートに自動生成。完了チェック・日程の手直し・会社ごとのExcel書き出しまで画面上でできる",
      "detail": {
        "overview": "社員1人の月次業務を1枚のHTMLガントチャートにまとめるツール。どのクライアントを載せるかは稼働請求時間ダッシュボードの担当表から毎月引き直し、作業の中身は業務管理ツールから取得する。画面上で完了チェック・並べ替え・カレンダーでの日付指定・メモができ、変更はその端末に自動で残る（閉じて開き直しても保持）。委託先へ渡すときは会社を1社だけ選んでExcelに書き出す仕組みで、他社の情報が混ざらないよう絞っていないと書き出せない。翌月は「引き継ぐ設定」を読み込めば、先月の並べ替えと日付をそのまま引き継げる。",
        "background": "業務管理ツールはクライアントごとにページを開かないと着手日・期限日・担当者が分からず、複数社を担当する社員は月の全体像を一目でつかめなかった。6月に作った版はAIがその都度HTMLを書き出す形だったため、受け取った人が直したい箇所に手を入れられず、翌月へ引き継ぐこともできなかった。9月に利用者の方から「自分の見やすいように直して使いたい」という要望が出たのを機に、生成を決まった手順（プログラム）に置き換え、受け取った人が画面上で直せる形に作り直した。",
        "tools": [
          "業務管理ツール連携（AI連携口）",
          "Python（取得データからHTMLを組み立てる処理）",
          "HTML / JavaScript（画面もExcel書き出しもブラウザ内だけで完結・外部送信なし）",
          "localStorage（ブラウザ内のデータ保存機能）",
          "ヘッドレスChrome（画面を開かずに動かす自動テスト・136項目）"
        ],
        "impact": {
          "before": "クライアントごとに業務管理ツールを個別に開いて確認（10社なら10回・月30〜50分）。日程の手直しや完了の印は手元に残せない",
          "after": "1画面で担当全社の月次業務を確認（1〜2分）。完了チェック・日程の手直し・会社ごとのExcel書き出しまで画面上で完結",
          "savedTime": "1社員あたり月約28〜48分・年間約6〜10時間削減（18名へ展開できれば年間約110〜180時間）"
        }
      }
    },
    {
    ...
    },
    {
      ...
      "category": "report",
      "status": "testing",
      "description": "マネーフォワード・freee・弥生会計の損益計算書（月次推移）を添付すると、数値をコードで確定させた月次PL分析レポートを返し、そのあと内容について質問できるツール",
      "detail": {
        "overview": "カスタムGPT「MF月次PLアナライザー」の再設計版。**マネーフォワード・freee・弥生会計の3ソフトに対応**（会計ソフトを選ぶ欄があり、選んだソフトと添付ファイルの中身が食い違ったら止まる）。損益計算書（月次推移）のファイルを添付すると、9つの節で構成された月次PL分析レポートを返す。最大の特徴は「数値はコードが確定させ、AIは文章だけを書く」構造で、AIには金額を直接書かせず引換券（プレースホルダ）だけを書かせ、コードが確定値に置き換える。会計ソフトの合計と明細の合算が1円でもズレたら処理を止める（fail-closed）。レポートが出たあとは、同じ画面で「人件費は売上の何％か」「販管費が増えたのは何が原因か」といった質問に答えられる（計算7種）。1つの会話につき1社に限定する構造にして、他社の数値が混ざる事故を仕組みで防いでいる。",
        "background": "現行のカスタムGPTには、無料版と有料版で品質が変わる／科目名・月・比較対象を取り違える／整合性チェックが自己申告で担保されない、といった課題があった。これらは指示文の書き方ではなく「AIに数値を読ませている」構造そのものが原因のため、数値の抽出・計算・検算をすべてコード側に移す設計に変更した。出力は担当者が顧問先に向けて話す材料になるため、事実誤認の代償が大きい点も設計の前提にしている。",
        "tools": [
          "Dify（AIワークフロー構築ツール）",
          "LLM（大規模言語モデル・gpt-5.4）",
          "Python（プログラミング言語）",
          "Codex CLI（設計レビュー）"
        ],
        "impact": {
          "before": "担当者が各自のChatGPTでカスタムGPTを動かす。環境によって品質が変わり、科目名や比較対象の取り違えが起きても気づけない",
          "after": "CSVを添付するだけで、誰が使っても同じ深さのレポートが出る。数値はコードが確定させ、合わないときは結果を出さずに止まる",
          "savedTime": "運用開始後に計測（想定＝月50〜100件・1件あたり数十分の分析準備を短縮）"
        }
      }
    },
    {
      ...
      "detail": {
        ...
        "background": "月次の入力作業に業務委託2名で合計19時間かかっており、時間単価契約のため稼働がそのままコストになっていた。実データを調べたところ、対象の現金出納帳だけで仕訳全体の約半分（年8,410行のうち4,298行）を占めていることが分かった。加えて、先方Excelの科目欄の末尾に付く数字が製造原価か販管費かの答えを既に持っていることも判明し、人が判断していると思われていた工程の大半が機械で置き換えられる見込みが立った。",
        "tools": [
          "Python",
          "Excel",
          "マネーフォワード クラウド会計"
    ...
    {
      "name": "お客様資料の依頼・回収管理",
      "nameEn": "Client Document Request and Collection Tracker",
      "category": "management",
      "status": "building",
      "description": "顧問先ごとに必要な書類を一覧化し、1件ずつ「未依頼・依頼中・取得済み」で管理。まだ届いていない書類だけを並べた依頼メールの下書きを自動で作る（送信は人が行う）",
      "detail": {
        "overview": "提携税理士法人の案件。月次・年次・税務イベントのたびに顧問先へお願いする書類は顧問先ごとに中身が違うため、必要書類を顧問先別に一覧化し、1件ずつ状態を管理する。時期が来たら仕組み側から声をかけ、まだ手元に無い書類だけを抜き出して依頼メールの下書きを自動で作る。送信は必ず人が行い、送った時点で自動的に「依頼済み」になる。届いたらワンタッチで「取得済み」にでき、返ってこないものは自動で目立たせる。",
        "background": "別案件のGR顧客カルテが全体凍結になった代わりに進めることになった案件。現状も資料一覧のExcelはあるが、バラバラで更新されない状態になっている。そのため、この案件が失敗する筋道は「更新されなくなる」の1本だと特定し、状態を変える操作の軽さを最優先の要件に置いた。顧問先ごとに会計ソフトが違うため、特定のソフトを前提にした既製品は全社への適用には使えない。",
        "tools": [
          "SharePointリスト（台帳の置き場・第一候補）",
          "Microsoft Teams（台帳をタブに貼る・お知らせ）",
          "Power Automate（標準コネクタのみ・追加費用なし）",
          "Office 365 Outlook（依頼メールの下書き作成）",
          "AI（LLM・最初の必要書類一覧の起こしと文面づくりだけ）"
        ],
        "impact": {
          "before": "依頼する書類の一覧は書面として存在せず、決算前の打合せで口頭でお願いしている。実測では資料は平均7日でそろい、催促も平均0〜1回で、時間そのものは詰まっていない。困っているのは、担当者ごとに依頼のやり方がバラバラで、誰が何を依頼したのかを全体で見る場所が無いこと（一覧のExcelはあるが更新されていない）",
          "after": "（目標）全社の資料依頼が、同じ1つの場所に載っている。まだ届いていない書類だけを並べた依頼メールの下書きが自動ででき、送った時点で自動的に「依頼済み」になる。3か月後も状態が更新され続けている",
          "savedTime": "依頼文を書く時間でも、資料がそろうまでの日数でも測らない（実測が平均7日・催促0〜1回で、短縮できる余地がほとんど無いため）。全社の依頼が同じ1つの場所に載っている割合と、3か月後も状態が更新され続けているかで測る"
        }
      }
    },
    {
      "name": "担当クライアント別 稼働請求時間ダッシュボード",
      "nameEn": "Per-Owner Client Billing Hours Dashboard",
      "category": "report",
      "status": "live",
      "description": "自分が担当しているクライアントの「今月ここまでの請求時間・契約時間・消化率・実働時間」を1画面で見られるようにする。kintoneの2つのアプリを毎日お昼12時に読み、社内のSharePointリストへ書き出す（急ぐときは更新ボタンですぐ更新できる）。消化率が80%を超えた担当先は、担当者本人のTeams個人チャットにも知らせる",
      "detail": {
        "overview": "支援先企業の正社員18名が、自分の担当クライアントの稼働状況を1画面で見られるようにする。kintoneの「稼働請求時間」アプリ（日々の作業記録）と「契約」アプリ（誰がどの会社の担当か）を毎日お昼12時のお昼に読み、担当者ごとに集計してSharePointのリストへ書き出す（急ぐときは更新ボタンを押せばすぐ更新される）。1行に契約時間・請求時間・消化率・実働時間・自分の実働・自分の実働率が並ぶので、契約の枠を使いすぎ／使わなすぎかと、その会社が自分の手を離れているかが同時に分かる。全員が同じリンクを開けば、それぞれ自分の担当分だけが出る。過去の月は別のリストに積み上がる。kintone本体には一切手を入れない（読み取りだけ）。",
        "background": "社員Aさんからの相談が発端。kintoneには集計表が12本あるが「自分の担当ぶんだけに絞る」ことができず、担当クライアントの状況を見るには1社ずつ開き直す必要があった。担当者の情報は契約アプリにしかなく、作業記録の「責任者」欄では今月210社中63社が空欄で、絞り込んでも4社しか出ない（正解は12社）。そのため2つのアプリを顧客コードで結合する形にした。試験運用に入ってから、社員Cさんの要望でG契約も対象に広げ、「工数のうち自分が手を動かした時間」とその割合も出すようにした（担当先が自分の手を離れているかを見るため）。",
        "tools": [
          "kintone（稼働請求時間アプリ・契約アプリ／閲覧のみ・書き込みなし）",
          "Python（毎週データを作り直す・品質ゲート付き）",
          "SharePointリスト（社内の置き場。ログインした人だけが見られる）",
          "PnP PowerShell（SharePointへの書き込み）",
          "Windowsのタスクスケジューラ（毎日お昼12時の自動実行＋更新ボタンの見張り）"
        ],
        "impact": {
          "before": "自分の担当クライアントの稼働状況をまとめて見る場所が無く、kintoneの集計表を1社ずつ開き直していた。契約時間に対してどれだけ使ったかも、その場では分からなかった。※実際に何社を月何回・1回何分見ているかは未実測（依頼者に確認予定）",
          "after": "**読み方の案内ページを開くだけ**で、上に「このデータは◯月◯日◯時 時点です」、下に自分の担当分だけの表が出る。契約時間・請求時間・消化率・実働時間・自分の実働・自分の実働率・請求差異が1行に並ぶ。消化率は「数字／今月記録なし／枠なし」を出し分け、色は月の進み具合と比べて付く。数字はkintoneの集計画面と同条件で109社すべて一致することを確認ずみ。更新は毎日お昼12時のお昼に自動（急ぐときは更新ボタン）。過去の月は別のリストに積み上がり、月が変わった週に先月ぶんを締め直す。",
          "savedTime": ""
        }
      }
    },
    {
      "name": "YouTubeショート 予約投稿・分析自動化",
      "nameEn": "YouTube Shorts Scheduling & Analytics Automation",
      "category": "management",
      "status": "live",
      "description": "Googleドライブの動画とスプレッドシートの投稿計画をもとに、YouTubeへ無人で予約投稿し、実績を集めて毎月の改善レポートまで自動で作る",
      "detail": {
        "overview": "複数の外部サービスを1本の流れにつないだ運用自動化。①Googleドライブに置かれた動画を検出し、②Googleスプレッドシートで管理する投稿日・タイトル・説明文・ハッシュタグを読み取り、③YouTubeへ予約投稿し、④公開後に固定コメントを自動投稿、⑤再生数・視聴維持率・流入元といった実績をYouTubeから取得してスプレッドシートとデータファイルへ書き戻す。Windowsのタスクスケジューラ（決まった時刻にプログラムを動かす機能）で毎日2回とPCにサインインした時に動き、毎月1日と15日には新しい動画の取り込みと投稿日程の組み直しも自動で行う。同じ動画の二重投稿・別の動画との取り違え・同じ日に2本入ることを防ぐ照合を備え、失敗やログインの期限切れはWindowsの通知で知らせる。人が触るのは月1回だけ。",
        "background": "動画を1本ずつ手でアップロードし、公開日時を設定し、説明文を書き、公開日にコメントを投稿する運用だった。週4本の頻度では作業が滞りやすく、投稿が止まれば配信も止まる。どの動画がなぜ伸びたのかを後から振り返る材料も残っていなかった。",
        "tools": [
          "Google ドライブ（動画ファイルの保管・取得）",
          "Google スプレッドシート（投稿計画・実績の管理）",
          "YouTube Data API（予約投稿・コメント投稿）",
          "YouTube Analytics API（再生数・視聴維持率・流入元の取得）",
          "OAuth 2.0（Googleアカウント連携の認証）",
          "Python（プログラミング言語）",
          "Windowsのタスクスケジューラ（無人での定時実行）"
        ],
        "impact": {
          "before": "1本ずつ手作業でアップロード・公開日時の設定・説明文の作成・コメントの投稿（週4本）",
          "after": "投稿・コメント・新しい動画の取り込み・日程の組み直しまで全自動。人が触るのは月1回・約1分（APIで取得できない指標の転記）と、他SNS用の書き出しで代用する新しい動画の中身確認のみ",
          "savedTime": "週4本ぶんの投稿作業がゼロに（残る手作業は月1回・約1分）"
        }
      }
    },
    {
      "name": "営業プレリサーチ",
      "nameEn": "Sales Pre-Meeting Research",
      "category": "report",
      "status": "building",
      "description": "会社名と都道府県を入れると、初回商談の前に知っておきたい会社の情報を1枚のレポートにまとめるツール。同じ名前の会社が複数あるときは候補から人が1社を選び、各項目に情報の出どころを付ける",
      "detail": {
        "overview": "旧版「社内営業プレリサーチ」の作り直し版。まず国の法人データ（gBizINFO）から同じ名前の会社を一覧にし、候補が複数なら人が1社を選ぶ（1社に決まったときも必ず1回確認する）。選んだ会社の公式サイトなどを読み、代表者名・設立・資本金などをまとめる。各項目には◎○△の印を機械が付け、どのページから取ったかを示す。売り手（自社）の設定を差し替えると、調べる観点とレポートの結論が自社向けに変わる。設定するのは、自社の商材・話す相手の役職・狙う会社の規模・探したい「兆し」（例：経理職の募集、資金調達や新拠点のニュース）。ツールはその兆しを採用ページやニュースから探して根拠の一文を添え、規模が合うかで「今アプローチすべきか」を判断し、商談で確かめる質問まで用意する。",
        "background": "旧版は2026年2月に全社公開したが、ほとんど使われなかった。検証すると、同じ名前の別の会社を調べてしまう・実在する中小企業で「公式サイトが見つかりません」と止まる・代表者名や設立年を事実と違う内容で書き、しかも「公式サイトで確認した事実」と表示する、という問題があった。会社の特定を公的データと人の確認に任せ、出どころの印を機械が付ける形に作り直した。",
        "tools": [
          "Dify（AIワークフロー構築ツール）",
          "gBizINFO（経済産業省の法人データAPI）",
          "Serper（Google検索のAPI）",
          "LLM（大規模言語モデル・gpt-5.4）",
          "Python（プログラミング言語・214項目の自動テスト）",
          "Codex CLI（設計レビュー）"
        ],
        "impact": {
