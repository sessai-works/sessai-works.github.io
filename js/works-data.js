...
var DASHBOARD_DATA = {
  ...
  "projects": [
    ...
    {
      "name": "議事録作成メーカー（支援先企業 管理部門用）",
      "nameEn": "Meeting Minutes Maker (支援先企業 Administration Team)",
      "category": "document",
      "status": "live",
      "description": "会議の文字起こし(Word)を送ると、クライアント向けと社内向けの二層議事録を自動で作り、SharePointに保存してTeamsで知らせるツール。支援先企業の管理部門用の受付窓口です",
      "detail": {
        "overview": "会議の文字起こし(Word)から、クライアント向けと社内向けの二層議事録を自動生成し、SharePoint保存・Teams通知まで一気通貫で行うツール。2026年7月に本番稼働を開始。受付フォームは窓口ごとに分かれていますが、裏側のしくみ（AI・保存・通知）は3窓口で共用しています。このカードは支援先企業 管理部門用の窓口です。",
