...
var DASHBOARD_DATA = {
  ...
  "projects": [
    ...
    },
    {
      ...
      "detail": {
        "overview": "毎月決まった営業日（第3営業日・第10営業日など、業務ごとにバラバラ）に発生する定例業務を、AI(Claude)が祝日込みで事前に正しい確定日付を計算したExcelマスタから、Power Automateが1行ずつOutlookカレンダーへV4で作成し、出席者へ招待(出欠あり)を送る。時間を決めた予定だけでなく、業務ごとに『終日(One-day)予定』も選んで登録できる(Outlook上部の帯で表示される終日予定)。営業日計算はマスタ生成時に確定させ、フローは『マスタを読んで転記する』だけに単純化。件名末尾の[AUTO:一意キー]＋当日取得＋未存在条件で再実行時の二重登録を防ぐ。",
